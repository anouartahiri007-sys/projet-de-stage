...
const RECENT_ACTIVITIES = (t: any, lang: string) => [
  {
    id: 1, name: lang === 'ar' ? 'ياسين بومدين' : 'Yassine Boumediane', desc: lang === 'ar' ? 'تم تحديث ملفه الإداري (الترقية)' : 'Dossier administratif mis à jour (Promotion)',
...
const PENDING_TASKS = (lang: string) => [
  { id: 1, title: lang === 'ar' ? 'المصادقة على لوائح الترقيات 2025' : 'Validation des listes de promotions 2025', priority: 'high', date: 'اليوم' },
  { id: 2, title: lang === 'ar' ? 'مراجعة ملفات التوظيف (مباراة 05/24)' : 'Revue des dossiers recrutement (Concours 05/24)', priority: 'medium', date: 'غداً' },
  { id: 3, title: lang === 'ar' ? 'تحديث الهيكل التنظيمي للمصلحة التقنية' : 'Mise à jour organigramme Service Technique', priority: 'low', date: '25 ماي' },
];
...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { label: lang === 'ar' ? 'التوظيف النشط' : 'Recrutements', value: '15', trend: '+3', color: 'purple', icon: <UserPlusIcon size={24} /> },
  ];
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
          <div className={lang === 'ar' ? 'text-right' : 'text-left'}>
            ...
            <p className="text-emerald-50/70 text-xl font-medium max-w-2xl leading-relaxed">
               {lang === 'ar' 
                 ? 'لوحة القيادة المركزية لتدبير الموارد البشرية. تتبع أداء الموظفين، الترقيات، والقرارات الإدارية بكفاءة عالية.'
                 : 'Tableau de bord central des RH. Suivi des performances, promotions et actes administratifs avec une efficacité optimale.'
               }
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-5 w-full lg:w-auto shrink-0">
             ...
             <button onClick={() => navigate('/rh/documents')} className="flex flex-col items-center justify-center gap-4 p-8 bg-white/10 backdrop-blur-md rounded-[2.5rem] border border-white/20 hover:bg-white/20 transition-all group shadow-2xl">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">{lang === 'ar' ? 'إصدار وثيقة' : 'Nouveau Doc'}</span>
             </button>
          </div>
        </div>
      </section>
        ...
        <div className="lg:col-span-4 space-y-10">
           ...
           <section className="gov-card p-10">
              ...
              <div className="grid grid-cols-2 gap-5">
                 {[
                   { label: t('hrReports'), icon: <BarChart3 size={28} />, path: '/rh/reports', color: 'blue' },
                   { label: t('employees'), icon: <Users size={28} />, path: '/rh/employees', color: 'emerald' },
                   { label: lang === 'ar' ? 'التعيينات' : 'Nominations', icon: <Star size={28} />, path: '/rh/acts/nomination', color: 'amber' },
                   { label: lang === 'ar' ? 'الأرشيف' : 'Archives', icon: <FileText size={28} />, path: '/rh/documents', color: 'slate' },
                 ].map((item, i) => (
                   <button
                     key={i}
              ...
              <div className="relative z-10 space-y-8">
                 ...
                 <div>
                    ...
                    <p className="text-blue-100/70 text-sm leading-relaxed font-bold">
                       {lang === 'ar' 
                         ? 'تمت معالجة 98% من ملفات التوظيف لهذا الأسبوع. نلاحظ تحسناً في الكفاءة الإدارية بنسبة 15% بعد تفعيل موديول العقود الجديد.'
                         : '98% des dossiers de recrutement traités cette semaine. Efficacité administrative en hausse de 15% via le nouveau module.'
                       }
                    </p>
                 </div>
                 <button onClick={() => navigate('/rh/reports')} className="w-full py-4 bg-white/10 hover:bg-white text-white hover:text-[#003366] rounded-2xl text-[10px] font-black uppercase tracking-[0.25em] transition-all border border-white/10">
                    {lang === 'ar' ? 'عرض التحليل الكامل' : 'Voir l\'analyse complète'}
                 </button>
              </div>
           </section>
